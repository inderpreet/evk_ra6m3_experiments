...
/* Actual period: 0.001 seconds. Actual duty: 50%. */.period_counts =
		...
		(timer_source_div_t) 0, .channel = 0, .p_callback = tmr0_callback,
...
const sci_uart_extended_cfg_t uart_one_cfg_extend = { .clock =
		...
		.noise_cancel = SCI_UART_NOISE_CANCELLATION_DISABLE, .rx_fifo_trigger =
				...
				&uart_one_baud_setting, .flow_control =
...
const uart_cfg_t uart_one_cfg = { .channel = 7, .data_bits = UART_DATA_BITS_8,
...
/* Instance structure to use this module. */
const uart_instance_t uart_one = { .p_ctrl = &uart_one_ctrl, .p_cfg =
		&uart_one_cfg, .p_api = &g_uart_on_sci };
